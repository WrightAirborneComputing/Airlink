...
class RcPacketSender:
    def __init__(self, name: str, port: int, interval_sec: float = 0.05, auto_start: bool = True):
        self.name = name
        self.port = port
    ...
    def set_channels(self, ch1, ch2, ch3, ch4, ch5, ch6, ch7, ch8):
        values = [ch1, ch2, ch3, ch4, ch5, ch6, ch7, ch8]
        self.channels = [max(1000, min(2000, int(v))) for v in values]
...
class RcPacketReceiver:
    def __init__(self, name: str, in_port: int, ack_port: int, auto_start: bool = True):
        self.name = name
    ...
    def _run(self):
        print(f"\r[{self.name}] RC receiver UDP {self.in_port}", flush=True)
        ...
        while self.running:
            try:
                ...
                print(
                    f"\r[{self.name}] frame={frame_count} "
...
class RcAckReceiver:
    def __init__(self, name: str, port: int, auto_start: bool = True):
        self.name = name
        self.port = port
        self.running = False
        self.thread = None
    ...
    def _run(self):
        ...
        while self.running:
            try:
                ...
                uplink_ms = (air_rx_time - tx_time) * 1000.0
                downlink_ms = (now - air_rx_time) * 1000.0
                total_latency_ms = (now - tx_time) * 1000.0
